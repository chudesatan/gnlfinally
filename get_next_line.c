...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/01/25 16:26:00 by fcorrupt          #+#    #+#             */
/*   Updated: 2019/02/24 15:04:58 by fcorrupt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	t_list		*tr_lst(t_list **lst_save, int fd)
{
	t_list	*tmp;
	t_list	*first;
...
static	char		*get_line_before_end(char **last_line_read)
{
	char	**str;
	int		i;
	char	*line;
...
static	int			reading_file(char **content, t_list *lst)
{
	char	*buf;
	int		size;
	char	*to_free;
...
int					get_next_line(const int fd, char **line)
{
	...
	if (!line || fd < 0 || BUFF_SIZE < 1)
		return (-1);
	lst = tr_lst(&lst_save, fd);
	if ((value = reading_file((char**)(&lst->content), lst)) == -1
	...
	*line = get_line_before_end((char**)(&lst->content));
	get_line_after_n((char**)(&lst->content));
	return (1);
}
